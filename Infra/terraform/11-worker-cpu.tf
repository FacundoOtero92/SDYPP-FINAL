...
# Instance Template para el pool
resource "google_compute_instance_template" "worker_cpu_tpl" {
  name_prefix  = "worker-cpu-${count.index}"
  ...
  network_interface {
    network = var.network
    access_config {}  # IP pública para docker pull (si no usás NAT)
  }
...
# Managed Instance Group (pool)
resource "google_compute_instance_group_manager" "worker_cpu_mig" {
  name               = "worker-cpu-mig"
  base_instance_name = "worker-cpu-${count.index}"
  zone               = var.zone
